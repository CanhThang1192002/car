...
export function CarsNewPage({ mode = 'create', carId }: { mode?: 'create' | 'edit'; carId?: number }) {
  ...
  const ui = useMemo(() => {
    const card =
      'rounded-xl border border-slate-200/70 bg-white shadow-sm shadow-slate-900/5 ring-1 ring-slate-900/5 dark:border-zinc-800/80 dark:bg-zinc-950 dark:shadow-none dark:ring-0'
    const cardBody = 'p-4 sm:p-5'
    const cardHeader =
      'flex items-start justify-between gap-3 border-b border-slate-200/70 bg-gradient-to-r from-indigo-50/60 to-transparent px-4 py-3 sm:px-5 dark:border-zinc-800/80 dark:from-indigo-950/20'
  ...
  const showroomsQ = useQuery({
    ...
    queryFn: async (): Promise<AdminShowroom[]> => {
      ...
      return (arr as any[]).map((s) => ({
        ...
        fullAddress: (s?.FullAddress ?? s?.fullAddress ?? null) as any,
        province: (s?.Province ?? s?.province ?? null) as any,
        district: (s?.District ?? s?.district ?? null) as any,
        streetAddress: (s?.StreetAddress ?? s?.streetAddress ?? null) as any,
      }))
    },
    staleTime: 60_000,
  })
  ...
  useEffect(() => {
    ...
    if (Array.isArray(features) && features.length) {
      const ids = features
        .map((f) => Number(f?.FeatureId ?? f?.featureId ?? 0))
        .filter((n) => Number.isFinite(n) && n > 0)
      if (ids.length) setFeatureIds(ids.join(','))
    }
  ...
  function addGalleryFiles(type: GalleryType, pickedFiles: File[]) {
    const incoming = pickedFiles ?? []
    if (!incoming.length) return

    setGallery((prev) => {
      const prevGroup = prev[type]
      const existing = prevGroup.files
      ...
      const nextFiles = [...existing, ...toAdd]
      const nextMetas = [
        ...prevGroup.metas.map((m) => ({ ...m, isMainImage: false, imageType: type })),
        ...toAdd.map(() => ({ title: '', description: '', imageType: type, isMainImage: false })),
      ]
  ...
  function removeGalleryImage(type: GalleryType, idx: number) {
    setGallery((prev) => {
      const g = prev[type]
      const nextFiles = g.files.filter((_, i) => i !== idx)
      const nextMetas = g.metas.filter((_, i) => i !== idx).map((m) => ({ ...m, isMainImage: false, imageType: type }))
      return { ...prev, [type]: { files: nextFiles, metas: nextMetas } }
    })
    setGalleryErrors((prev) => {
      ...
      return { ...prev, [type]: Object.keys(next).length ? next : undefined }
    })
  }
  ...
  const createM = useMutation({
    mutationFn: async () => {
      ...
      fd.append('DisplayStatus', displayStatus)
  ...
  return (
    <div className="w-full bg-slate-50/60 px-4 py-6 text-slate-900 dark:bg-zinc-950/10 dark:text-zinc-100">
      <div className="w-full max-w-none">
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
              ...
              <div className="mt-1 text-sm text-slate-600 dark:text-zinc-300">
                {mode === 'edit'
                  ? 'Dữ liệu được load sẵn từ backend. Chỉnh xong bấm Lưu.'
                  : 'Nhập thông tin cơ bản, tồn kho, thông số, pricing versions và ảnh.'}
              </div>
            </div>

            <div className="flex flex-wrap items-center gap-2">
              <button type="button" disabled={createM.isPending} onClick={() => navigate('/cars')} className={ui.btnGhost}>
                Huỷ
              </button>
        ...
        {mode === 'edit' && detailQ.isError ? (
          <div className="mb-4 text-sm text-rose-600">Không tải được dữ liệu xe.</div>
        ) : null}
        ...
        <div className="grid grid-cols-1 gap-6 pb-10">
          <section className={ui.card}>
            <div className={ui.cardHeader}>
              <div>
                <h2 className={ui.cardTitle}>Thông tin chính</h2>
                <div className={ui.cardSub}>Dữ liệu bảng Cars + ảnh chính + mô tả.</div>
              </div>
            ...
            <div>
              <label className={ui.label}>Tên xe</label>
              <input
                value={name}
                onChange={(e) => setName(e.target.value)}
                placeholder="VD: VinFast VF 7 Plus"
                className={ui.control}
              />
            </div>
            <div>
              <label className={ui.label}>Hãng</label>
              <input
                value={brand}
            ...
            <div>
              <label className={ui.label}>Năm</label>
              <input
                value={year}
            ...
            <div>
              <label className={ui.label}>Màu</label>
              <input
                value={color}
            ...
            <div>
              <label className={ui.label}>ODO (km)</label>
              <input
                value={mileage}
            ...
            <div>
              ...
              <div className="mt-1 text-xs text-slate-500 dark:text-zinc-400">
                Nếu có pricing versions, backend sẽ set lại giá thấp nhất.
              </div>
            </div>
            ...
            <div>
              ...
              <div className="mt-1 text-xs text-slate-500 dark:text-zinc-400">
                Sales tạo có thể bị ép về PendingApproval ở backend (tuỳ rule).
              </div>
            </div>
              </div>
          ...
          <section className={ui.card}>
            <div className={ui.cardHeader}>
              <div>
                <h2 className={ui.cardTitle}>Tồn kho</h2>
                <div className={ui.cardSub}>Thiết lập kho mặc định hoặc thêm nhiều showroom.</div>
              </div>
            ...
            <div>
              ...
              >
                ...
                {!showroomsQ.isLoading && showroomOptions.length === 0 ? (
                  <option value={showroomId}>Không có showroom</option>
                ) : null}
                {showroomOptions.map((s) => (
                  <option key={s.showroomId} value={String(s.showroomId)}>
                    {s.name}
                    {s.fullAddress ? ` - ${s.fullAddress}` : ''}
                  </option>
                ))}
              </select>
              {showroomsQ.isError ? (
                <div className="mt-1 text-xs text-red-600">Không tải được danh sách showroom</div>
              ) : null}
            </div>
            <div>
              <label className={ui.label}>Số lượng mặc định</label>
              <input
                value={quantity}
            ...
            <div>
              ...
              <div className="mt-5 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <div>
                  ...
                  <div className="mt-1 text-xs text-slate-500 dark:text-zinc-400">
                    Nếu để trống, backend có thể dùng kho mặc định.
                  </div>
                </div>
            <button
              type="button"
              onClick={() => setInventories((p) => [...p, { showroomId, quantity, displayStatus }])}
                  className={ui.btn}
            >
              + Thêm kho
            </button>
              ...
              {inventories.map((row, idx) => (
                  ...
                  <button
                    ...
                    onClick={() => setInventories((p) => p.filter((_, i) => i !== idx))}
                            className={'md:col-span-1 ' + ui.btnGhost}
          ...
          <section className={ui.card}>
            <div className={ui.cardHeader}>
              <div>
                <h2 className={ui.cardTitle}>Thông số</h2>
                <div className={ui.cardSub}>Nhập theo từng dòng (category / name / value).</div>
              </div>
          ...
          <section className={ui.card}>
            <div className={ui.cardHeader}>
              <div>
                <h2 className={ui.cardTitle}>Phiên bản / Giá</h2>
                <div className={ui.cardSub}>Backend sẽ lấy giá thấp nhất để set vào Cars (nếu có).</div>
              </div>
            ...
            {pricing.map((p, idx) => (
                ...
                <input
                  value={p.versionName}
                  onChange={(e) => setPricing((prev) => prev.map((x, i) => (i === idx ? { ...x, versionName: e.target.value } : x)))}
                ...
                />
                        <label className="md:col-span-2 flex items-center gap-2 rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 text-sm text-slate-800 dark:border-zinc-800 dark:bg-zinc-900 dark:text-zinc-200">
                          <input
                            type="checkbox"
                            checked={p.isActive}
                            onChange={(e) =>
                              setPricing((prev) => prev.map((x, i) => (i === idx ? { ...x, isActive: e.target.checked } : x)))
                            }
                          />
                          Active
                </label>
          ...
          <section className={ui.card}>
            <div className={ui.cardHeader}>
              <div>
                <h2 className={ui.cardTitle}>Ảnh phụ</h2>
                <div className={ui.cardSub}>
                  Meta sẽ map theo thứ tự file trong từng nhóm, khi submit sẽ ghép theo thứ tự nhóm:{' '}
                  <span className="font-mono">{GALLERY_GROUPS.map((g) => g.type).join(' → ')}</span>
                </div>
              </div>
            ...
            {GALLERY_GROUPS.map((group) => {
              ...
              return (
                      <div
                        key={group.type}
                        className="rounded-xl border border-slate-200/70 bg-white p-4 shadow-sm shadow-slate-900/5 dark:border-zinc-800/80 dark:bg-zinc-950 dark:shadow-none"
                      >
                        <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
                          <div>
                            <div className="flex flex-wrap items-center gap-2">
                              <div className="text-sm font-semibold text-slate-900 dark:text-zinc-100">{group.label}</div>
                              <span className={ui.badge}>ImageType: {group.type}</span>
                            </div>
                  ...
                  <div className="mt-3 grid grid-cols-1 gap-4 md:grid-cols-2">
                    <div>
                      <label className={ui.label}>Chọn ảnh</label>
                      <input
                        type="file"
                        multiple
                        accept="image/*"
                        onChange={(e) => {
                          ...
                          addGalleryFiles(group.type, files)
                          // allow picking same file again if needed
                          e.currentTarget.value = ''
                        }}
                              className={ui.control}
                      />
                      <div className="mt-1 text-xs text-slate-500 dark:text-zinc-400">
                        Meta ảnh sẽ map theo thứ tự file đã chọn trong nhóm này.
                      </div>
                    </div>
                  </div>
                  ...
                  {st.files.length > 0 ? (
                              ...
                              >
                                <div className="flex items-center justify-between gap-3">
                                  <div className="min-w-0">
                                    <div className="truncate text-sm font-medium text-slate-900 dark:text-zinc-100">{f.name}</div>
                                  ...
                                  <button
                                    type="button"
                                    onClick={() => removeGalleryImage(group.type, idx)}
                                    className={ui.btnDanger}
                          ...
                          <div className="mt-2 grid grid-cols-1 gap-2 md:grid-cols-12">
                            <input
                              ...
                              placeholder={group.defaultTitlePlaceholder ?? 'Title (tuỳ chọn)'}
                                    className={'md:col-span-3 ' + (errs[idx] ? ui.controlError : ui.control)}
                            />

                            <div className="md:col-span-3 rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm text-slate-700 dark:border-zinc-800 dark:bg-zinc-950 dark:text-zinc-200">
                              {group.type}
                            </div>
                            ...
                            <textarea
                              ...
                              placeholder="Description (hỗ trợ xuống dòng)"
                                    className={'md:col-span-6 ' + (errs[idx] ? ui.textareaError : ui.textarea)}
                            />
                    ...
                    <div className="mt-3 text-xs text-slate-500 dark:text-zinc-400">Chưa chọn ảnh cho nhóm này.</div>
                  )}
                </div>
              )
            })}
        ...
        {/* <div className="sticky bottom-4 z-20">
          <div className="rounded-xl border border-slate-200/70 bg-white/80 p-3 shadow-lg shadow-slate-900/10 backdrop-blur dark:border-zinc-800/80 dark:bg-zinc-950/65 dark:shadow-none">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              <div className="text-sm text-slate-600 dark:text-zinc-300">
                {createM.isPending ? 'Đang tạo xe, vui lòng chờ…' : 'Sẵn sàng tạo xe mới.'}
              </div>
              <div className="flex items-center justify-end gap-2">
                ...
                <button type="button" disabled={createM.isPending} onClick={() => createM.mutate()} className={ui.btnPrimary}>
                  {createM.isPending ? 'Đang tạo...' : 'Tạo xe'}
                </button>
              </div>
            </div>
          </div>
