        ...
        <div className="mx-auto max-w-screen-2xl space-y-10 px-5 py-14 sm:px-6 lg:px-8 lg:py-16">
          ...
          <div className="overflow-hidden rounded-lg border border-slate-200 bg-white">
            ...
            <dl className="divide-y divide-slate-200/90">
              {content.specsRows.map((row, idx) => {
                const category = (row.category ?? '').trim()
                return (
                  <div
                    key={`${category}-${row.label}-${idx}`}
                    className="grid gap-1 bg-slate-100/90 px-4 py-3.5 sm:grid-cols-[minmax(0,0.9fr)_minmax(0,1fr)_minmax(0,1.1fr)] sm:gap-6 sm:px-6"
                  >
                    <dt className="text-xs font-semibold uppercase tracking-wide text-slate-500 sm:text-sm sm:font-semibold sm:normal-case sm:tracking-normal">
                      <span className="sm:hidden">Danh mục: </span>
                      {category || '—'}
                    </dt>
                    <dt className="text-sm font-semibold text-slate-800">
                      <span className="sm:hidden text-xs font-semibold uppercase tracking-wide text-slate-500">
                        Thông số:{' '}
                      </span>
                      {row.label}
                    </dt>
