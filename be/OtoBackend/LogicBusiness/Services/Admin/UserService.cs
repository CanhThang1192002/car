...
namespace LogicBusiness.Services.Admin
{
    public class UserService : IUserService
    {
            ...
            int? currentUserShowroomId)
        {
            ...
            var showroom = await _showroomRepository.GetByIdAsync(request.ShowroomId);
            if (showroom == null) return (false, "Showroom không tồn tại trong hệ thống!");
            ...
            {
                if (status != "Active" && status != "Inactive") return (false, "Status không hợp lệ (chỉ nhận Active/Inactive).");
                user.Status = status;
            }

            await _userRepository.UpdateUserAsync(user);
            return (true, $"Cập nhật thông tin người dùng #{user.UserId} thành công!");
        }
    }
}
